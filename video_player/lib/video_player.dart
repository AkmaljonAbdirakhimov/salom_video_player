...
/// Extended options for video player which includes view type settings.
class ExtendedVideoPlayerOptions extends VideoPlayerOptions {
  ...
  ///
  /// [VideoViewType.textureView] uses Flutter's Texture widget (default).
  /// [VideoViewType.platformView] uses platform-specific views which might
  /// have better performance or additional features on certain platforms.
...
class VideoPlayerController extends ValueNotifier<VideoPlayerValue> {
  ...
  Future<void> initialize() async {
    ...
    // Determine which view type to use
    // If ExtendedVideoPlayerOptions is provided, use its viewType
    // Otherwise, use platformView for iOS and textureView for other platforms as default
    ...
    void eventListener(VideoEvent event) {
      ...
      switch (event.eventType) {
        ...
        case VideoEventType.pipEntered:
          // No state changes needed for PiP events in the controller
          // The event will be propagated to listeners via isInPictureInPictureModeStream
          break;
        case VideoEventType.pipExited:
          // No state changes needed for PiP events in the controller
          // The event will be propagated to listeners via isInPictureInPictureModeStream
          break;
  ...
  /// Enters Picture-in-Picture mode if the device and platform support it.
  ///
  /// Returns true if PiP mode was successfully entered, false otherwise.
  ///
  /// This feature is currently supported only on iOS and Android.
  /// On unsupported platforms, this will return false.
  Future<bool> enterPictureInPicture() async {
    ...
    try {
      final bool success =
          await _videoPlayerPlatform.enterPictureInPictureMode(_textureId);
      return success;
    } catch (e) {
      return false;
    }
  ...
  /// Checks if the video player is currently in Picture-in-Picture mode.
  ///
  /// Returns true if the player is in PiP mode, false otherwise.
  Future<bool> isInPictureInPictureMode() async {
    ...
    try {
      return await _videoPlayerPlatform.isInPictureInPictureMode(_textureId);
    } catch (e) {
      return false;
    }
  }

  /// A stream that emits events when the Picture-in-Picture mode changes.
  ///
  /// The stream emits a boolean value: true when PiP is active, false when it's not.
  Stream<bool> get isInPictureInPictureModeStream {
    // Transform event subscription to filter only PiP events
    final StreamController<bool> pipController =
        StreamController<bool>.broadcast();

    // Handle cleanup when the stream is cancelled
    StreamSubscription<dynamic>? eventSubscription;
    pipController.onCancel = () {
    ...
    if (eventSubscription != null) {
      _eventSubscription!.onData((dynamic event) {
        if (event is VideoEvent) {
          if (event.eventType == VideoEventType.pipEntered) {
            pipController.add(true);
          } else if (event.eventType == VideoEventType.pipExited) {
            pipController.add(false);
          }
        }
      });
    }

    return pipController.stream;
  }
}
