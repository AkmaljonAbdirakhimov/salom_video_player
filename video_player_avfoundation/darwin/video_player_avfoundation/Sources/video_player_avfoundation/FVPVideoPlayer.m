...
// Add PiP implementation

#if TARGET_OS_IOS
- (void)setupPictureInPictureWithPlayerLayer:(AVPlayerLayer *)playerLayer {
  if (!playerLayer || !_player || _disposed) {
    NSLog(@"Warning: Cannot setup PiP - invalid player or layer");
    return;
  }
  
  // Check if PiP is supported
  if (![FVPVideoPlayer isPictureInPictureSupported]) {
    NSLog(@"Warning: PiP is not supported on this device");
    return;
  }
  
  // Create and configure the PiP controller if not already created
  if (!_pipController) {
    if (@available(iOS 9.0, *)) {
      ...
      NSLog(@"PiP controller successfully set up");
    }
  }
}

- (BOOL)enterPictureInPictureMode {
  if (!_pipController || _disposed) {
    NSLog(@"Error: Cannot enter PiP mode - PiP controller not initialized or player is disposed");
    return NO;
  }
  ...
  if (@available(iOS 9.0, *)) {
    if ([_pipController isPictureInPictureActive]) {
      // Already in PiP mode
      NSLog(@"Already in PiP mode");
      return YES;
    }
    
    if ([_pipController isPictureInPicturePossible]) {
      NSLog(@"Starting PiP mode");
      [_pipController startPictureInPicture];
      return YES;
    } else {
      NSLog(@"PiP is not possible at this moment");
    }
  }
...
+ (BOOL)isPictureInPictureSupported {
  if (@available(iOS 9.0, *)) {
    ...
    NSLog(@"PiP support check: %@", supported ? @"Supported" : @"Not supported");
    return supported;
  }
  return NO;
}
...
- (void)pictureInPictureControllerDidStartPictureInPicture:(AVPictureInPictureController *)pictureInPictureController {
  // Send event to notify Dart that PiP mode has started
  NSLog(@"PiP did start");
  if (_eventSink) {
    _eventSink(@{@"event" : @"pipEntered"});
  }
}

- (void)pictureInPictureControllerDidStopPictureInPicture:(AVPictureInPictureController *)pictureInPictureController {
  // Send event to notify Dart that PiP mode has ended
  NSLog(@"PiP did stop");
  if (_eventSink) {
    _eventSink(@{@"event" : @"pipExited"});
  }
}

- (void)pictureInPictureController:(AVPictureInPictureController *)pictureInPictureController 
     restoreUserInterfaceForPictureInPictureStopWithCompletionHandler:(void (^)(BOOL))completionHandler {
  // This is called when the user taps the restore button in PiP mode
  // We can use this to restore the app's UI to the video playback screen if needed
  NSLog(@"PiP restore UI requested");
