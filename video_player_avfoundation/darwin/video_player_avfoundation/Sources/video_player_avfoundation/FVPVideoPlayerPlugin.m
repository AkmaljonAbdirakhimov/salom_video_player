...
+ (void)registerWithRegistrar:(NSObject<FlutterPluginRegistrar> *)registrar {
  [FVPVideoCacheManager initializeCache]; // Initialize the cache manager
...
- (nullable FVPTextureBasedVideoPlayer *)texturePlayerWithOptions:
  ...
  if (options.asset) {
    ...
    if ([[FVPVideoCacheManager shared] isVideoCached:uri]) {
      NSLog(@"Using cached video asset for %@", uri);
      AVAsset *cachedAsset = [[FVPVideoCacheManager shared] getAssetForURL:uri];
      return [[FVPTextureBasedVideoPlayer alloc] initWithAVAsset:cachedAsset
                                                   frameUpdater:frameUpdater
                                                    displayLink:displayLink
                                                      avFactory:_avFactory
                                                      registrar:self.registrar
      ...
      // Fall back to normal URL loading
      return [[FVPTextureBasedVideoPlayer alloc] initWithURL:[NSURL URLWithString:uri]
                                              frameUpdater:frameUpdater
                                               displayLink:displayLink
...
- (nullable FVPVideoPlayer *)platformViewPlayerWithOptions:(nonnull FVPCreationOptions *)options {
  ...
  if (options.asset) {
    ...
    // Check if the video is cached
    if ([[FVPVideoCacheManager shared] isVideoCached:uri]) {
      NSLog(@"Using cached video asset for platform view: %@", uri);
      AVAsset *cachedAsset = [[FVPVideoCacheManager shared] getAssetForURL:uri];
      
      // Use the new initWithAVAsset: method directly
      return [[FVPVideoPlayer alloc] initWithAVAsset:cachedAsset
                                           avFactory:_avFactory
                                           registrar:self.registrar];
    } else {
      // Fall back to normal URL loading
...
- (nullable NSString *)startDownload:(nonnull NSString *)url error:(FlutterError **)error {
  NSString *urlId = [[FVPVideoCacheManager shared] startDownload:url];
  return urlId;
}

- (nullable NSNumber *)cancelDownload:(nonnull NSString *)url error:(FlutterError **)error {
  BOOL result = [[FVPVideoCacheManager shared] cancelDownload:url];
  return @(result);
}

- (nullable NSNumber *)removeDownload:(nonnull NSString *)url error:(FlutterError **)error {
  BOOL result = [[FVPVideoCacheManager shared] removeDownload:url];
  return @(result);
}

- (nullable NSNumber *)getDownloadProgress:(nonnull NSString *)url error:(FlutterError **)error {
    double progress = [[FVPVideoCacheManager shared] getDownloadProgress:url];
    long bytesDownloaded = [[FVPVideoCacheManager shared] getBytesDownloaded:url];
    
    FVPDownloadProgress *progressObj = [FVPDownloadProgress makeWithUrl:url
                                                               progress:progress
...
- (nullable NSString *)getCachedVideoPath:(nonnull NSString *)url error:(FlutterError **)error {
  NSString *path = [[FVPVideoCacheManager shared] getCachedVideoPath:url];
  return path;
}

- (nullable NSNumber *)getMaxConcurrentDownloads:(FlutterError **)error {
  int count = [[FVPVideoCacheManager shared] getMaxConcurrentDownloads];
  return @(count);
}

- (nullable FVPDownloadStateBox *)getDownloadState:(nonnull NSString *)url error:(FlutterError **)error {
  NSInteger state = [[FVPVideoCacheManager shared] getDownloadState:url];
  FVPDownloadState enumState;
  
  switch (state) {
    case 0:
      enumState = FVPDownloadStateInitial;
      break;
    case 1:
      enumState = FVPDownloadStateDownloading;
      break;
    case 2:
      enumState = FVPDownloadStateDownloaded;
      break;
    case 3:
      enumState = FVPDownloadStateFailed;
      break;
    default:
      enumState = FVPDownloadStateInitial;
      break;
  ...
  return [[FVPDownloadStateBox alloc] initWithValue:enumState];
}

// Implement PiP methods
- (NSNumber *)enterPictureInPictureModeForPlayer:(NSInteger)playerID error:(FlutterError **)error {
#if TARGET_OS_IOS
  // Create a proper NSNumber key from playerID
  NSNumber *playerKey = @(playerID);
  FVPVideoPlayer *player = self.playersByIdentifier[playerKey];
  if (!player) {
    if (error) {
      *error = [FlutterError errorWithCode:@"player-not-found"
                                  message:@"No video player with that ID"
                                  details:nil];
    }
    return @NO;
  }
  
  BOOL success = [player enterPictureInPictureMode];
  return @(success);
#else
  if (error) {
    *error = [FlutterError errorWithCode:@"not-supported"
...
- (NSNumber *)isPictureInPictureSupported:(FlutterError **)error {
#if TARGET_OS_IOS
  return @([FVPVideoPlayer isPictureInPictureSupported]);
#else
  if (error) {
    *error = [FlutterError errorWithCode:@"not-supported"
                                message:@"Picture-in-Picture is only supported on iOS"
...
- (NSNumber *)isInPictureInPictureModeForPlayer:(NSInteger)playerID error:(FlutterError **)error {
#if TARGET_OS_IOS
  // Create a proper NSNumber key from playerID
  NSNumber *playerKey = @(playerID);
  FVPVideoPlayer *player = self.playersByIdentifier[playerKey];
  ...
  BOOL isInPipMode = [player isInPictureInPictureMode];
  return @(isInPipMode);
#else
  if (error) {
    *error = [FlutterError errorWithCode:@"not-supported"
